...
```{r}
library(ggplot2)
filter(crimes, `Crm.Cd.Desc`=="BURGLARY") %>%
ggplot( aes(y=`Weapon.Desc`)) +
  geom_bar()

filter(crimes, `Crm.Cd.Desc`=="BATTERY - SIMPLE ASSAULT") %>%
ggplot( aes(y=`Weapon.Desc`)) +
  geom_bar()

filter(crimes, `Weapon.Desc`=="STRONG-ARM (HANDS, FIST, FEET OR BODILY FORCE)") %>%
ggplot( aes(y=`Crm.Cd.Desc`)) +
  geom_bar()

filter(crimes, `Weapon.Desc`=="") %>%
ggplot( aes(y=`Crm.Cd.Desc`)) +
  geom_bar()
